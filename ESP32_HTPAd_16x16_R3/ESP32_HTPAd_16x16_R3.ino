...
struct characteristics {
  ...
  unsigned char ATCactive;
  unsigned char ATCPos;
...
void sort_data() {
  ...
  ******************************************************************************************************************/
  ...
  for (int i = 0; i < DevConst.NumberOfBlocks; i++) {
    // block top half
    sum += (unsigned short)(RAMoutput[i][DevConst.ATCPos] << 8 | RAMoutput[i][DevConst.ATCPos + 1]);
    // block bottom half
    sum2 += (unsigned short)(RAMoutput[2 * DevConst.NumberOfBlocks - i + 1][DevConst.ATCPos] << 8 | RAMoutput[2 * DevConst.NumberOfBlocks - i + 1][DevConst.ATCPos + 1]);
  }
...
void checkUDP() {
  ...
  if (packetSize) {
    ...
    if (strcmp(packetBuffer, "Calling HTPA series devices") == 0) {
      ...
      udp.print("HTPA series responsed! I am Arraytype 17"); // 00 for 8x8
      //udp.print(arraytype);
