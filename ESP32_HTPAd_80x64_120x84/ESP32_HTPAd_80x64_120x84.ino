/*** PROGRAMM INFO***************************************************************************************
  ...
  name:           ESP_appshield.ino
  version/date:   2.0 / 09 Feb 2022
...
void setup() {
  ...
  TimerLib.setInterval_us(ISR, timert + TIMER_OFFSET);
  //*******************************************************************
...
void read_eeprom() {
  ...
  n = 0;
  // top half
  for (int i = 0; i < (unsigned short)(DevConst.RowPerBlock); i++) {
  ...
  n = 0;
  for (int i = (unsigned short)(DevConst.RowPerBlock); i < (unsigned short)(DevConst.RowPerBlock * 2); i++) {
    vddcompgrad[m][n] = read_EEPROM_byte(E_VDDCOMPGRAD + 2 * i + 1) << 8 | read_EEPROM_byte(E_VDDCOMPGRAD + 2 * i);
    vddcompoff[m][n] = read_EEPROM_byte(E_VDDCOMPOFF + 2 * i + 1) << 8 | read_EEPROM_byte(E_VDDCOMPOFF + 2 * i);
    n++;
    if (n ==  DevConst.PixelPerRow) {
...
void sort_data() {
  ...
  for (int m = 0; m < DevConst.RowPerBlock; m++) {
    for (int n = 0; n < DevConst.PixelPerRow; n++) {
      ...
      ******************************************************************************************************************/
      if (new_offsets) {
        if ((picnum < ELOFFSETS_FILTER_START_DELAY) && (!use_eloffsets_buffer) ) {
          // top half
  ...
  ******************************************************************************************************************/
  if (switch_ptat_vdd == 1) {
    ...
    for (int i = 0; i < DevConst.NumberOfBlocks; i++) {
      ...
      // block bottom half
      sum += (unsigned short)(RAMoutput[2 * DevConst.NumberOfBlocks - i][DevConst.PTATPos] << 8 | RAMoutput[2 * DevConst.NumberOfBlocks - i][DevConst.PTATPos + 1]);
    }
  ...
  ******************************************************************************************************************/
  if (switch_ptat_vdd == 0) {
    ...
    for (int i = 0; i < DevConst.NumberOfBlocks; i++) {
      ...
      // block bottom half
      sum += (unsigned short)(RAMoutput[2 * DevConst.NumberOfBlocks - i][DevConst.VDDPos] << 8 | RAMoutput[2 * DevConst.NumberOfBlocks - i][DevConst.VDDPos + 1]);
    }
...
void print_menu() {
  Serial.println("\n\n\n***************************************************");
  Serial.println("Application Shield v2.0                 /_/eimann");
