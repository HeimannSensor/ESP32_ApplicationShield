/*** PROGRAMM INFO***************************************************************************************
  ...
  name:           ESP32_HTPAd_80x64.ino
  version/date:   2.3 / 21 Aug 2023
...
unsigned char nrofdefpix, gradscale, vddscgrad, vddscoff, epsilon, lastepsilon, arraytype, bitstoshift;
...
unsigned char read_block_num = START_WITH_BLOCK;
...
void read_eeprom() {
  ...
  bitstoshift = 12-(mbit_calib&0x0F);
      ...
      ******************************************************************************************************************/
      if (picnum % ELOFFSETS_BUFFER_SIZE == 1) {
        if ((!eloffset[m][n]) || (picnum < ELOFFSETS_FILTER_START_DELAY)) {
          ...
          // bottom half
          eloffset[2 * DevConst.RowPerBlock - 1 - m][n] = (unsigned short)((RAMoutput[DevConst.NumberOfBlocks + 1][pos] << 8 | RAMoutput[DevConst.NumberOfBlocks + 1][pos + 1]) << bitstoshift);
...
void checkSerial() {
  ...
  switch (serial_input) {
    ...
    case 's':
      ReadingRoutineEnable = 0;
      TimerLib.clearTimer();
      // to sleep sensor set configuration register to 0x00
      // |    RFU    |   Block   | Start | VDD_MEAS | BLIND | WAKEUP |
      // |  0  |  0  |  0  |  0  |   0   |    0     |   0   |    0   |
      write_sensor_byte(CONFIGURATION_REGISTER, 0x00);
      delay(100);
      break;
    ...
    case 'w':
      ReadingRoutineEnable = 0;
      TimerLib.clearTimer();
      // to wake up and start sensor set configuration register to 0x09
      // |    RFU    |   Block   | Start | VDD_MEAS | BLIND | WAKEUP |
      // |  0  |  0  |  0  |  0  |   1   |    0     |   0   |    1   |
